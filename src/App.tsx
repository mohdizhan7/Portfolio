...
// Magnetic link (subtle hover pull)
function Magnet(props: ComponentProps<'a'>){
...
export default function App(){
  ...
  // dots: highlight current section
  ...
  useEffect(()=>{
    const io=new IntersectionObserver(entries=>{
      entries.forEach(e=>{ if(e.isIntersecting) setActive(e.target.id as typeof active); });
    },{threshold:.45});
  ...
  return (
    <main>
      ...
      {/* frosted sticky nav + accent swatches */}
      <div className="nav">
        <div className="nav-inner">
          ...
          <nav style={{display:'flex',gap:12,alignItems:'center'}}>
            ...
            <div className="swatches" aria-label="Accent colors">
              <button type="button" className="swatch indigo" onClick={()=>setAccent('indigo')} title="Indigo" />
              <button type="button" className="swatch green"  onClick={()=>setAccent('green')}  title="Green" />
              <button type="button" className="swatch pink"   onClick={()=>setAccent('pink')}   title="Pink" />
            </div>
          </nav>
        </div>
      </div>
      ...
      <section className="hero" ref={heroRef as any}>
        ...
        />
        <motion.div style={{ y: textY }} initial={{opacity:0}} animate={{opacity:1}} transition={{duration:.6, delay:.05}}>
      ...
      <section id="experience" className="section">
        <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{once:true,amount:.5}}>Experience</motion.h2>

        <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{once:true,amount:.45}}>
      ...
      <section id="skills" className="section">
        <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{once:true,amount:.5}}>Skills &amp; Certifications</motion.h2>
      ...
      <section id="contact" className="section">
        <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{once:true,amount:.5}}>Contact</motion.h2>
